class UsersClubsController < ApplicationController
  ...
  def index
    @users_clubs = @club.users_clubs.active_first.first_name
  ...
  def new
    @users_club = UsersClub.new(:club_id => @club.id)
  end
  ...
  def create
    ...
    if @users_club.save
      ...
      redirect_to(users_clubs_path(:club_id => @club.id))
    else
      render('new')
    end
  end
  ...
  def update
    ...
    if @users_club.update_attributes(user_params)
      ...
      redirect_to(users_club_path(@users_club, :club_id => @club.id))
    else
      render('edit')
    end
  end
